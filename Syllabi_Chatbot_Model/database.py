...
def extract(file_path):
    ...
    course_name = re.search(r'course name:\s*(.*)', course_info, re.IGNORECASE).group(1).strip()
    course_instructor = re.search(r'course instructor:\s*(.*)', course_info, re.IGNORECASE).group(1).strip()
    course_description = re.search(r'course brief description:\s*(.*)', course_info, re.IGNORECASE).group(1).strip()
    course_location = re.search(r'course location:\s*(.*)', course_info, re.IGNORECASE).group(1).strip()
    course_start_date = re.search(r'course start date:\s*(.*)', course_info, re.IGNORECASE).group(1).strip()
    course_end_date = re.search(r'course end date:\s*(.*)', course_info, re.IGNORECASE).group(1).strip()

    result = [course_name,course_instructor,course_description,course_location,course_start_date,course_end_date]
    return result
    ...
    try:
        ...
        with connection.cursor() as cursor:
            ...
            sql_query = """
                            ...
                            (COURSE_NAME, COURSE_INSTRUCTOR,COURSE_INFO, COURSE_LOCATION, COURSE_START_DATE, COURSE_END_DATE) 
                            VALUES (%s, %s, %s, %s, %s, %s);
                        """
            cursor.execute(sql_query, (index_list[0], index_list[1], index_list[2], index_list[3], index_list[4], index_list[5]))
            connection.commit()
